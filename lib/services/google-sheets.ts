...
async function debugGoogleKey() {
  ...
  console.log("Private key looks multiline:", privateKey.includes("-----BEGIN"));
  console.log("Private key ends with footer:", privateKey.includes("-----END PRIVATE KEY-----"));
  console.log("Contains non-ascii chars:", /[^\x00-\x7F]/.test(privateKey) ? true : false);
